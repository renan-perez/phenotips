...
public class Orphanet extends AbstractOWLSolrVocabulary
{
    ...
    private static final Map<String, String> STATIC_SOLR_PARAMS = getStaticSolrParams();
    ...
    {
        final SolrParams dynamicSolrParams = produceDynamicSolrParams(input, maxResults, sort, customFilter);

        final ImmutableList.Builder<VocabularyTerm> resultBuilder = ImmutableList.builder();

        for (final SolrDocument doc : search(dynamicSolrParams, STATIC_SOLR_PARAMS)) {
    ...
    /**
     * Returns the static Solr parameters.
     *
     * @return static Solr parameters for the query
     */
    private static Map<String, String> getStaticSolrParams()
    {
        ...
        // Add the static field Solr parameters.
        paramBuilder.put(DisMaxParams.PF, "name^20 nameSpell^36 nameExact^100 namePrefix^30 "
            + "alternative_term^15 alternative_termSpell^25 alternative_termExact^70 alternative_termPrefix^20 "
            + "def^10 defSpell^15 "
            + "text^3 textSpell^5");
        paramBuilder.put(DisMaxParams.QF, "id^100 idStub^75 "
            + "name^10 nameSpell^18 nameStub^5 "
            + "alternative_term^6 alternative_termSpell^10 alternative_termStub^3 "
